...
创建 [GitHub Personal Access Token](https://github.com/settings/tokens)（公开 Star 只需读公开信息；含私有 Star 需相应权限），在终端导出：
...
> 开发端口 **4173**。结束占用该端口的进程：`npm run dev:stop`（支持 macOS / Linux / Windows）。
